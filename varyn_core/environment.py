...
class Environment:
    ...
    def lookup(self, name, lineno):
        ...
        while env is not None:
            ...
            env = env.parent
        raise VarynError("Tanımlanmamış Değişken (NameError)", f"'{name}' tanımlanmamış bir değişken.", lineno)
    ...
    def assign(self, name, value, lineno, modifier=None):
        ...
        if modifier is not None:
            ...
            if name in self.values and self.is_constant(name):
                raise VarynError("Sabit Hatası (ConstantError)", f"'{name}' bir sabittir ve değeri değiştirilemez.", lineno)
            ...
            if target_mod:
                ...
                if modifier != target_mod and target_mod != 'değişken' and modifier != 'değişken':
                    raise VarynError("Tip Hatası (TypeError)", f"'{name}' değişkeninin veri türü değiştirilemez (Mevcut: '{target_mod}', Verilen: '{modifier}').", lineno)
