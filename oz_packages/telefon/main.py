...
def ilerleme(yuzde):
    try:
        ...
        val = 0
    _append_element({"type": "ilerleme", "val": val})
    return True
...
def harita(konum, boylam=None):
    if boylam is not None:
        try:
            lat = float(konum)
            lng = float(boylam)
            _append_element({"type": "harita", "lat": lat, "lng": lng})
        except Exception:
...
def sekme(elemanlar, aktif_sekme_indeksi=0):
    elems = [str(x) for x in elemanlar] if isinstance(elemanlar, list) else [str(elemanlar)]
    try:
        idx = int(float(aktif_sekme_indeksi))
    except Exception:
        idx = 0
    _append_element({"type": "sekme", "items": elems, "active_index": idx})
    return True

def kaydirici(etiket, min_deger=0, max_deger=100, varsayilan=50):
    try:
        mn = int(float(min_deger))
        mx = int(float(max_deger))
        val = int(float(varsayilan))
    except Exception:
        mn = 0
        mx = 100
        val = 50
