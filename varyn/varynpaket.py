# -*- coding: utf-8 -*-
"""
Varyn Geliştirici Araçları (varynpaket.py)
...
def show_help():
    print("=== varynpaket - Varyn Eklenti Geliştirici Araçları ===")
    print("Kullanım:")
    print("  python3 varynpaket.py oluştur <paket_adı> -> Yeni bir eklenti şablonu oluşturur")
    print("  python3 varynpaket.py test                -> Mevcut dizindeki test.varyn dosyasını çalıştırarak paketi test eder")
    print("  python3 varynpaket.py paketle             -> Paketi yayınlanmaya hazır .zip dosyası haline getirir")
    print("  python3 varynpaket.py yardım              -> Yardım menüsünü gösterir")
...
def cmd_create(name):
    ...
    meta = {
        ...
        "yazar": "gelistirici",
        "tur": "python",
        "aciklama": "Yeni oluşturulmuş harika bir Varyn eklentisi.",
        "izinler": [],
        "bagimliliklar": []
    }
    with open(os.path.join(name, "varynpaket.json"), "w", encoding="utf-8") as f:
...
def selam_ver(isim):
    \"\"\"
    Varyn içinden doğrudan 'selam_ver(\"Ahmet\")' olarak çağrılabilir.
    \"\"\"
...
"""
    ...
    # 3. README.md
    readme_content = f"""# {name} Varyn Eklentisi

Bu eklenti Varyn eklenti ekosistemi için otomatik olarak oluşturulmuştur.
...
değişken sonuc = selam_ver("Varyn Test")
yazdır("Dönen Değer: " + sonuc)
"""
    with open(os.path.join(name, "test.varyn"), "w", encoding="utf-8") as f:
        f.write(test_oz_content)
        
    print(f"✨ Başarılı: '{name}' eklenti şablonu ve dosyaları başarıyla oluşturuldu!")

def cmd_test():
    if not os.path.isfile("varynpaket.json"):
        print("❌ Hata: Mevcut dizinde 'varynpaket.json' bulunamadı! Lütfen bir paket dizininde olduğunuzdan emin olun.")
        return
        
    if not os.path.isfile("test.varyn"):
        print("❌ Hata: Test için 'test.varyn' dosyası bulunamadı!")
        return
        
    print("🧪 Paket test ediliyor...")
    
    # Varyn interpreter'ını dinamik çağır
    try:
        ...
        with open("test.varyn", "r", encoding="utf-8") as f:
            test_code = f.read()
            
        print("--- Çalıştırılan Varyn Test Kodu ---")
        print(test_code)
        print("-----------------------------------")
...
def cmd_pack():
    if not os.path.isfile("varynpaket.json"):
        print("❌ Hata: Mevcut dizinde 'varynpaket.json' bulunamadı!")
        return
        
    try:
        with open("varynpaket.json", "r", encoding="utf-8") as f:
            meta = json.load(f)
    except Exception as e:
        print(f"❌ Hata: 'varynpaket.json' okunamadı: {str(e)}")
        return
