# -*- coding: utf-8 -*-
"""
Varyn Paket Yöneticisi Çekirdeği (package_manager.py)
...
import hashlib
import re
from varyn.repository import fetch_package_data

_CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
_PROJECT_ROOT = os.path.abspath(os.path.join(_CURRENT_DIR, ".."))
LOCAL_PACKAGES_DIR = os.path.join(_PROJECT_ROOT, "varyn_packages")
GLOBAL_PACKAGES_DIR = os.path.abspath(os.path.expanduser("~/.varyn/packages"))
...
def get_installed_package_meta(name):
    """
    Sistemde kurulu olan bir paketin varynpaket.json meta verilerini okur.
    """
    ...
    for parent in [LOCAL_PACKAGES_DIR, GLOBAL_PACKAGES_DIR]:
        ...
        if not pkg_path.startswith(normalized_parent + os.sep) and pkg_path != normalized_parent:
            continue
        meta_file = os.path.join(pkg_path, "varynpaket.json")
...
def verify_package_signature(pkg_name):
    ...
    for root, dirs, files in os.walk(pkg_path):
        ...
        for f in files:
            if f == "varynpaket.json":
                continue # varynpaket.json imza içermez, bu yüzden hariç bırakılır
...
def install_package(pkg_name, target="local", installing_set=None):
    ...
    try:
        ...
        with open(os.path.join(pkg_dest_path, "varynpaket.json"), "w", encoding="utf-8") as f:
