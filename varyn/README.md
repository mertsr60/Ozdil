# Varyn Paket ve Eklenti Sistemi Dokümantasyonu 📚

Varyn, Python tabanlı eklentileri (plugins) ve yerel Varyn kütüphanelerini yönetebilen, tam uyumlu bir paket yönetim ekosistemine sahiptir. Bu sistem, Python'ın `pip` + `import` mantığıyla çalışır ve güvenlik için AST tabanlı bir Sandbox denetimi barındırır.
...
- `varyn/varyn.py`: Varyn Interpreter/Compiler köprüsü.
- `varyn/varynpip.py`: `varynpip` paket yöneticisi komut satırı arayüzü (CLI).
- `varyn/plugin_api.py`: Python eklentilerinin Varyn ile entegre olmasını sağlayan dinamik API ve Event (Olay) sistemi.
- `varyn/package_manager.py`: Bağımlılık çözme, sürüm uyumluluğu ve imza doğrulaması yapan çekirdek kütüphane.
- `varyn/sandbox.py`: Güvenli çalışma ortamı sunan, AST tabanlı Python kod analizcisi.
- `varyn/repository.py` & `repository.json`: Paket deposu ve metadata veritabanı simülasyonu.

---

## 1. Varyn Paketi Nasıl Yapılır? (`tur: "varyn"`)

Yerel bir Varyn paketi, yalnızca Varyn dilinde yazılmış kodlardan oluşur.

### Adım 1: Klasör ve varynpaket.json Hazırlama
Öncelikle `varyn_packages/` veya küresel paketler dizininde paket ismiyle bir klasör oluşturun ve içine `varynpaket.json` yerleştirin:
...
```json
{
  ...
  "tur": "varyn",
  "aciklama": "Geometrik şekillerin alan hesaplamalarını yapan Varyn paketi.",
  "izinler": [],
  "bagimliliklar": []
}
```

### Adım 2: Varyn Kodunu Yazma (`geometri.varyn` veya `main.varyn`)
...
### Adım 3: Kullanım
Varyn ana programında bu paketi şu şekilde içe aktarabilir ve kullanabilirsiniz:
...
Python eklentileri, Varyn'in sınırlarını genişleterek Python'ın geniş kütüphane ekosistemini Varyn içinde kullanmanızı sağlar.

### Adım 1: varynpaket.json Tanımlama
...
Eklentinin Varyn'e fonksiyon veya komut sunabilmesi için bir `plugin()` fonksiyonu barındırması ve bu fonksiyonun dışa aktarılacak arabirimleri içeren bir sözlük (dictionary) dönmesi gerekir.
...
Eklentiler, Varyn'in çalışma zamanı olaylarını dinleyebilir. `plugin_api` içinde sunulan olay kancaları şunlardır:

1. `program_basladi`: Varyn kodu yorumlanmaya başlamadan hemen önce tetiklenir.
...
def baslangic_kancasi():
    print("[Eklenti Olayı] Varyn programı şu an başladı!")
...
Varyn, eklentilerin sisteme zarar vermesini engellemek için **AST (Abstract Syntax Tree)** tabanlı son derece güvenli bir süzgeç barındırır. 
...
1. Bir paket kurulurken veya `varynpaket paketle` komutuyla paketlenirken, paketin içerdiği tüm dosyaların içeriği ve yolları birleştirilerek SHA256 hash'i (imza) üretilir.
2. Bu imza `varynpaket.json` içerisine yazılır ve merkezi depoda saklanır.
...
## 6. Geliştirici ve Paketleme Araçları (`varynpaket.py`)
...
```bash
python3 varynpaket.py oluştur <eklenti_adi>
```
Bu komut, belirtilen isimde bir klasör oluşturur ve içine standart `varynpaket.json`, `main.py`, `README.md` ve `test.varyn` şablonlarını otomatik yerleştirir.

### 2) Eklentiyi Test Etme:
Paket dizininizin içindeyken şu komutu çalıştırarak testleri koşturabilirsiniz:
```bash
python3 varynpaket.py test
```
Bu komut `test.varyn` dosyasını Varyn interpreter'ı ile çalıştırarak çıktıları gösterir.

### 3) Dağıtıma Hazır Paketleme:
```bash
python3 varynpaket.py paketle
```
