...
from .errors import OzdilError, ReturnException, InputRequestException
...
class VirtualMachine:
    ...
    def init_builtins(self):
        ...
        # Core Built-ins
        self.global_env.define('yazdır', wrap_value(oz_yazdir))
        self.global_env.define('yazdir', wrap_value(oz_yazdir))
        self.global_env.define('girdi', wrap_value(oz_girdi))
        self.global_env.define('input', wrap_value(oz_girdi))
            ...
            try:
                ...
                if not handled:
                    if isinstance(e, OzdilError):
                        raise e
                    raise OzdilError("Yürütme Hatası (RuntimeError)", f"Sanal makinede hata oluştu: {str(e)}", 1, e)
    ...
    def execute_instruction(self, opcode, operand, frame):
        if opcode == 'LOAD_CONST':
            ...
            try:
                ...
                raise OzdilError("Tür Hatası (TypeError)", "Döngü kurulamaz, veri yinelenebilir (iterable) değil.", 1)
            ...
            if isinstance(callable_obj, OzFunction):
                ...
                raise OzdilError("Tür Hatası (TypeError)", f"'{callable_obj}' nesnesi çağrılamaz.", 1)
    ...
    def perform_binary_op(self, left, right, op):
        ...
        try:
            if op == '+': res = left_native + right_native
                ...
                if right_native == 0:
                    raise OzdilError("Sıfıra Bölme Hatası (ZeroDivisionError)", "Bir sayı sıfıra bölünemez veya mod alınamaz.", 1)
                if op == '/':
                ...
                raise OzdilError("Tür Hatası (TypeError)", f"Bilinmeyen operatör '{op}'", 1)
            return wrap_value(res)
        except Exception as e:
            if isinstance(e, OzdilError):
                raise e
            raise OzdilError("Tür Hatası (TypeError)", f"'{op}' işlemi için uyumsuz veri türleri ({getattr(left, 'get_type_name', lambda: type(left).__name__)()} ve {getattr(right, 'get_type_name', lambda: type(right).__name__)()})", 1)
    ...
    def perform_unary_op(self, val, op):
        ...
        try:
            if op == '+': res = +val_native
                ...
                raise OzdilError("Tür Hatası (TypeError)", f"Bilinmeyen tekli operatör '{op}'", 1)
            return wrap_value(res)
        except Exception as e:
            raise OzdilError("Tür Hatası (TypeError)", f"'{op}' işlemi için uyumsuz veri türü ({getattr(val, 'get_type_name', lambda: type(val).__name__)()})", 1)
