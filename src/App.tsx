...
export default function App() {
  ...
  const handleRunTests = async () => {
    ...
    setPanelHeight("normal");
    setPanelTab("terminal");
    showToast("Otomatik motor testleri koşturuluyor...", "info");
    ...
    try {
      ...
      const data = await response.json();
      setResults({
        translated: "# Çekirdek Motor Otomatik Birim Testleri Raporu\n# tests/run_tests.py koşturuldu.",
        ast: null,
        output: data.output,
        error: data.success ? null : "Bazı testler başarısız oldu!"
      });
      if (data.success) {
        showToast("Tüm testler başarıyla geçti! ✓", "success");
      } else {
        showToast("Bazı testler başarısız oldu!", "error");
      }
    } catch (err) {
      showToast("Testler çalıştırılırken bağlantı hatası.", "error");
    } finally {
      setIsRunning(false);
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <nav className="hidden md:flex items-center gap-1.5 text-xs font-semibold text-zinc-600 dark:text-zinc-300">
            ...
            <div className="relative">
              ...
              {activeMenu === "run" && (
                <div className="absolute top-9 left-0 w-64 bg-white dark:bg-zinc-950 border border-zinc-200/80 dark:border-zinc-850 rounded-xl shadow-xl py-1.5 z-50 text-[11px] animate-in fade-in slide-in-from-top-2 duration-150" onMouseLeave={() => setActiveMenu(null)}>
                  ...
                  <button onClick={() => { handleRunTests(); setActiveMenu(null); }} disabled={isRunning} className="w-full text-left px-3 py-2 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center justify-between transition text-zinc-700 dark:text-zinc-300 disabled:opacity-50 border-t border-zinc-100 dark:border-zinc-900 mt-1 pt-2">
                    <span className="font-medium flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400">
                      <Cpu className="w-3.5 h-3.5 shrink-0" />
                      <span>Motor Testlerini Koştur</span>
                    </span>
                    <span className="text-indigo-400 font-mono text-[9px]">Birim Testi</span>
                  </button>
                </div>
              )}
            </div>
